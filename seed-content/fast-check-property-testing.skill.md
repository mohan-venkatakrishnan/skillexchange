...
category: Testing
description: Stop hand-picking the three inputs you already thought of and let the runner hunt for the one you didn't — round-trip properties, invariants, and shrinking that hands you a two-character counterexample instead of a 400-element blob. Prevents the whole class of bug that lives in the gaps between your example tests: the empty string, the lone surrogate, the negative zero, the reordering that only breaks on the fourth operation.
...
For the common case, `@fast-check/vitest` collapses the ceremony: `test.prop([fc.string(), fc.string()])('concat length is additive', (a, b) => { expect((a + b).length).toBe(a.length + b.length) })`.
...
const lineItem = fc.record({ sku: fc.stringMatching(/^[A-Z]{3}-\d{4}$/), qty: fc.integer({ min: 1, max: 99 }), unit: money })
const invoice = fc.record({
  id: fc.uuid(),
  issuedAt: fc.date({ min: new Date('2020-01-01'), max: new Date('2030-01-01'), noInvalidDate: true }),
  items: fc.array(lineItem, { minLength: 1, maxLength: 20 }),
})
```
...
Bound recursive shapes with `fc.letrec` plus an explicit `maxDepth` of 3-5. Leave it unbounded and you'll eventually generate a 10,000-node tree, wait 90 seconds for one run, and abandon a shrink path nobody has the patience to watch.
...
it('parse ∘ serialize is the identity on query strings', () => {
  const queryParams = fc.dictionary(
    ...
    fc.oneof(fc.string(), fc.array(fc.string(), { maxLength: 3 })), { maxKeys: 8 })
...
**Invariant** — a truth that survives every operation. Over an `invoice` and any array of discount rules, `applyDiscounts` must return a total that is an integer, never negative, and never above the subtotal. Three assertions, one property, and it catches both the float-cents bug and the stacking-discounts-below-zero bug in the same run.

**Oracle / metamorphic** — compare against a slow-but-obviously-correct reference. `search(buildIndex(docs), q)` must return exactly what `docs.filter(d => d.body.includes(q))` returns: the naive linear scan *is* the spec, and the fast index has to match it for every generated corpus. When no oracle exists, reach for a metamorphic relation instead — you may not know what `rank(docs, q)` should return, but you know appending an irrelevant document must not reorder the existing results.
...
Counterexample: [{"amountCents":1,"currency":"JPY"}]   // shrunk 6 times
Got error: expected '¥0' to be '¥1'
```

Reproduce it exactly by passing both back — `fc.assert(fc.property(money, checkFormat), { seed: -1063857102, path: '42:2:1:0' })`. The `path` jumps straight to the shrunk case with no re-hunting.
...
Raising `numRuns` has sharply diminishing returns against a bad arbitrary. If your bug needs two fields to collide and each is drawn from 10,000 values, no run count saves you — narrow the range until collisions happen. This is the same reason stateful and model-based suites need a deliberately tiny alphabet: forty unique keys against a capacity-3 LRU cache means no evictions, no re-hits, and no bugs found. Check what you're actually generating before tuning anything:
...
- **Model-based `fc.commands` without a `toString`, or with a huge key space.** No `toString` and the shrunk sequence prints as `[object Object]`, destroying the one artifact you came for: a readable four-step repro. Too many distinct keys and the operations never interfere, so the state machine explores nothing.
...
const table = header.chain((cols) => fc.record({
...
describe('csv round trip', () => {
  ...
  it('generates the interesting shapes it claims to', () => {
    ...
    // has-quote-cell 41.2% · has-newline-cell 33.8% ← the branch that breaks parsers · plain 6.9%
  })
})
